...
const queryFn$9 = () => core.fetchSigner();
...
function useSigner() {
  ...
  const queryClient = reactQuery.useQueryClient();
  React__namespace.useEffect(() => {
    const unwatch = core.watchSigner(signer => queryClient.setQueryData(queryKey$9(), signer));
    return unwatch;
  }, [queryClient]);
  return signerQuery;
}
...
function useContractInfiniteReads(_ref5) {
  ...
  const queryKey_ = React__namespace.useMemo(() => queryKey$8([{
    cacheKey,
    overrides
  }]), [cacheKey, overrides]);
...
function useContractRead(_ref4) {
  ...
  const queryKey_ = React__namespace.useMemo(() => queryKey$7([{
    addressOrName,
    args,
    chainId,
    contractInterface,
...
function useContractReads(_ref4) {
  ...
  const chainId = useChainId();
  const queryKey_ = React__namespace.useMemo(() => queryKey$6([{
    allowFailure,
    contracts,
    overrides
  }, {
